...
import { WigoswapFactory } from "../types"
...
describe("WigoswapFactory", () => {
  ...
  async function fixture([wallet, other]: Wallet[]) {
    const tmp = await ethers.getContractFactory("WigoswapFactory")
    const factory = await tmp.deploy(wallet.address)
    return { factory: factory as WigoswapFactory, wallet, other }
  }
  ...
  async function createPair(factory: WigoswapFactory, tokens: [string, string]) {
    const pairContract = await ethers.getContractFactory("WigoswapPair")
    ...
    await expect(factory.createPair(tokens[0], tokens[1])).to.be.reverted // Wigoswap: PAIR_EXISTS
    await expect(factory.createPair(tokens[1], tokens[0])).to.be.reverted // Wigoswap: PAIR_EXISTS
  ...
  it("createPair:gas", async () => {
    ...
    expect(receipt.gasUsed).to.eq(2479009)
  })

  it("setFeeTo", async () => {
    const { factory, wallet, other } = await loadFixture(fixture)
    await expect(factory.connect(other).setFeeTo(other.address)).to.be.revertedWith("Wigoswap: FORBIDDEN")
  ...
  it("setFeeToSetter", async () => {
    const { factory, wallet, other } = await loadFixture(fixture)
    await expect(factory.connect(other).setFeeToSetter(other.address)).to.be.revertedWith("Wigoswap: FORBIDDEN")
    await factory.setFeeToSetter(other.address)
    expect(await factory.feeToSetter()).to.eq(other.address)
    await expect(factory.setFeeToSetter(wallet.address)).to.be.revertedWith("Wigoswap: FORBIDDEN")
  })
})
