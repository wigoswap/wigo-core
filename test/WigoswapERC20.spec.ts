...
describe("WigoswapERC20", () => {
  ...
  it("name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH", async () => {
    ...
    expect(name).to.equal("Wigo LPs")
    expect(await token.symbol()).to.equal("Wigo-LP")
  ...
  it("transfer", async () => {
    ...
    expect(await token.balanceOf(wallet.address)).to.equal(TOTAL_SUPPLY.sub(TEST_AMOUNT))
  ...
  it("permit", async () => {
    ...
    const { v, r, s } = wallet._signingKey().signDigest(Buffer.from(digest.slice(2), "hex"))
